# TITLE: BRANCHED EVOLUTION AND MACROEVOLUTION-LIKE TRANSITIONS SHAPE HEAD AND NECK SQUAMOUS CELL CARCINOMA ACROSS SPACE AND STAGE

# SHORT TITLE: BRANCHED AND MACROEVOLUTION-LIKE EVOLUTION IN HNSCC
...
This repository provides the computational code used to support the study **“Branched evolution and macroevolution-like transitions shape head and neck squamous cell carcinoma across space and stage.”** The included scripts are designed for evolutionary-mode classification based on subclonal topology and abundance features, and for spatially explicit three-dimensional simulation of mutation-driven subclonal evolution in HNSCC.
...
The model first applies a rule-based linear-tree check. Non-linear cases are then classified using a monotonic XGBoost binary classifier for `Near-neutral` versus `Branched`.
